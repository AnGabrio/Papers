...
#2)Disaggregated total costs: time-specific variables
#put together all disaggregated cost data 
...
#M2) assume HRU missing at time j if fully missing at that time / else treat it as 0 if only partially missing at that time
...
TC_1 <- PBS_dataset_quest_cc_hru_AD$AG_C1/1000
TC_2 <- PBS_dataset_quest_cc_hru_AD$AG_C2/1000
group <- PBS_dataset_quest_cc_hru_AD$group
TC_0 <- PBS_dataset_quest_cc_hru_AD$AG_C0/1000
hist(TC_2[group==0])
summary(TC_2[group==0])
hist(TC_1[group==0])
summary(TC_1[group==0])
hist(TC_0[group==0])
summary(TC_0[group==0])
hist(TC_2[group==1])
summary(TC_2[group==1])
hist(TC_1[group==1])
summary(TC_1[group==1])
hist(TC_0[group==1])
summary(TC_0[group==1])
#check fit of standard distributions to observed data for AG_TC and TC_0 (since missing values in both)
#add small constant to TC_j to avoid 0
library(fitdistrplus)
plotdist(TC_2[complete.cases(TC_2)], histo = TRUE, demp = TRUE)
descdist(TC_2[complete.cases(TC_2)], boot = 1000)
fw <- fitdist(TC_2[complete.cases(TC_2)]+0.01, "weibull")
fg <- fitdist(TC_2[complete.cases(TC_2)]+0.01, "gamma")
fln <- fitdist(TC_2[complete.cases(TC_2)]+0.01, "lnorm")
fn <- fitdist(TC_2[complete.cases(TC_2)]+0.01, "norm")
summary(fw)
summary(fg)
summary(fln)
summary(fn)
...
plotdist(TC_1[complete.cases(TC_1)], histo = TRUE, demp = TRUE)
descdist(TC_1[complete.cases(TC_1)], boot = 1000)
fw <- fitdist(TC_1[complete.cases(TC_1)]+0.01, "weibull")
fg <- fitdist(TC_1[complete.cases(TC_1)]+0.01, "gamma")
fln <- fitdist(TC_1[complete.cases(TC_1)]+0.01, "lnorm")
fn <- fitdist(TC_1[complete.cases(TC_1)]+0.01, "norm")
summary(fw)
summary(fg)
summary(fln)
summary(fn)
...
library(R2jags)
set.seed(2345)
datalist<-list("TC_2","TC_1","TC_0","N","group")
inits1 <- list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = 1)
inits2 <- list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = 2)
params<-c("alpha0","alpha1","alpha2","mu_TC0","sd_TC0","sd_TC1","sd_TC2","mu_TC1","mu_TC2","tau_TC0","tau_TC1","tau_TC2","log_lik_TC1","log_lik_TC2")
...
datalist<-list("TC_2","TC_1","TC_0","N","group","EQ5D0","EQ5D1","EQ5D2")
params<-c("alpha0","alpha1","alpha2","alpha3","mu_TC2","sd_TC2","mu_TC1","sd_TC1","sd_TC0","mu_TC0","tau_TC2","tau_TC1","tau_TC0",
          "beta0_EQ5D0","beta1_EQ5D0","beta0_EQ5D1","beta1_EQ5D1","beta2_EQ5D1","beta3_EQ5D1",
          "beta0_EQ5D2","beta1_EQ5D2","beta2_EQ5D2","beta3_EQ5D2", "mu_EQ5D2","sd_EQ5D2",
          "mu_EQ5D1","sd_EQ5D1", "mu_EQ5D0","sd_EQ5D0")
...
res_s2_method1 <- readRDS("res_s2_method1.rds")
jagsModel_norm <- res_s2_method1$res_s2_method1_norm$jagsModel_norm
jagsModel_lnorm <- res_s2_method1$res_s2_method1_lnorm$jagsModel_lnorm
jagsModel_gamma <- res_s2_method1$res_s2_method1_gamma$jagsModel_gamma

#compare model fit via IC and PPC
library(loo)
log_lik_TC1_norm <- jagsModel_norm$BUGSoutput$sims.list$log_lik_TC1
log_lik_TC2_norm <- jagsModel_norm$BUGSoutput$sims.list$log_lik_TC2
ic_waic_TC1_norm <- waic(log_lik_TC1_norm)
ic_waic_TC2_norm <- waic(log_lik_TC2_norm)
ic_looic_TC1_norm <- loo(log_lik_TC1_norm)
ic_looic_TC2_norm <- loo(log_lik_TC2_norm)
ic_waic_norm <- ic_waic_TC1_norm$estimates[,1] + ic_waic_TC2_norm$estimates[,1]
ic_looic_norm <- ic_looic_TC1_norm$estimates[,1] + ic_looic_TC2_norm$estimates[,1]

log_lik_TC1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$log_lik_TC1
log_lik_TC2_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$log_lik_TC2
ic_waic_TC1_lnorm <- waic(log_lik_TC1_lnorm)
ic_waic_TC2_lnorm <- waic(log_lik_TC2_lnorm)
ic_looic_TC1_lnorm <- loo(log_lik_TC1_lnorm)
ic_looic_TC2_lnorm <- loo(log_lik_TC2_lnorm)
ic_waic_lnorm <- ic_waic_TC1_lnorm$estimates[,1] + ic_waic_TC2_lnorm$estimates[,1]
ic_looic_lnorm <- ic_looic_TC1_lnorm$estimates[,1] + ic_looic_TC2_lnorm$estimates[,1]

log_lik_TC1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$log_lik_TC1
log_lik_TC2_gamma <- jagsModel_gamma$BUGSoutput$sims.list$log_lik_TC2
ic_waic_TC1_gamma <- waic(log_lik_TC1_gamma)
ic_waic_TC2_gamma <- waic(log_lik_TC2_gamma)
ic_looic_TC1_gamma <- loo(log_lik_TC1_gamma)
ic_looic_TC2_gamma <- loo(log_lik_TC2_gamma)
ic_waic_gamma <- ic_waic_TC1_gamma$estimates[,1] + ic_waic_TC2_gamma$estimates[,1]
ic_looic_gamma <- ic_looic_TC1_gamma$estimates[,1] + ic_looic_TC2_gamma$estimates[,1]
...
TC2_g0 <- TC_2[group==0]
TC2_g1 <- TC_2[group==1]
TC1_g0_obs <- TC1_g0[complete.cases(TC1_g0)]
TC1_g1_obs <- TC1_g1[complete.cases(TC1_g1)]
TC2_g0_obs <- TC2_g0[complete.cases(TC2_g0)]
TC2_g1_obs <- TC2_g1[complete.cases(TC2_g1)]
mu_tc1_obs_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC1[,is.na(TC_1)==FALSE & group==0]
mu_tc1_obs_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC1[,is.na(TC_1)==FALSE & group==1]
mu_tc2_obs_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC2[,is.na(TC_2)==FALSE & group==0]
mu_tc2_obs_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC2[,is.na(TC_2)==FALSE & group==1]
sd_tc1_obs_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC1[,1]
sd_tc1_obs_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC1[,2]
sd_tc2_obs_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC2[,1]
sd_tc2_obs_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC2[,2]
TC1_obs_g0_rep_norm <- matrix(NA, nrow = jagsModel_norm$n.iter, ncol = dim(mu_tc1_obs_g0_norm)[2])
TC1_obs_g1_rep_norm <- matrix(NA, nrow = jagsModel_norm$n.iter, ncol = dim(mu_tc1_obs_g1_norm)[2])
TC2_obs_g0_rep_norm <- matrix(NA, nrow = jagsModel_norm$n.iter, ncol = dim(mu_tc2_obs_g0_norm)[2])
TC2_obs_g1_rep_norm <- matrix(NA, nrow = jagsModel_norm$n.iter, ncol = dim(mu_tc2_obs_g1_norm)[2])
set.seed(8361299)
for(i in 1:jagsModel_norm$n.iter){
    TC1_obs_g0_rep_norm[i,] <- rnorm(dim(mu_tc1_obs_g0_norm)[2], mu_tc1_obs_g0_norm[i,], sd_tc1_obs_g0_norm[i])
    TC1_obs_g1_rep_norm[i,] <- rnorm(dim(mu_tc1_obs_g1_norm)[2], mu_tc1_obs_g1_norm[i,], sd_tc1_obs_g1_norm[i])
    TC2_obs_g0_rep_norm[i,] <- rnorm(dim(mu_tc2_obs_g0_norm)[2], mu_tc2_obs_g0_norm[i,], sd_tc2_obs_g0_norm[i])
    TC2_obs_g1_rep_norm[i,] <- rnorm(dim(mu_tc2_obs_g1_norm)[2], mu_tc2_obs_g1_norm[i,], sd_tc2_obs_g1_norm[i])
}
mu_tc1_obs_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC1[,is.na(TC_1)==FALSE & group==0]
mu_tc1_obs_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC1[,is.na(TC_1)==FALSE & group==1]
mu_tc2_obs_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC2[,is.na(TC_2)==FALSE & group==0]
mu_tc2_obs_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC2[,is.na(TC_2)==FALSE & group==1]
sd_tc1_obs_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC1[,1]
sd_tc1_obs_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC1[,2]
sd_tc2_obs_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC2[,1]
sd_tc2_obs_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC2[,2]
TC1_obs_g0_rep_lnorm <- matrix(NA, nrow = jagsModel_lnorm$n.iter, ncol = dim(mu_tc1_obs_g0_lnorm)[2])
TC1_obs_g1_rep_lnorm <- matrix(NA, nrow = jagsModel_lnorm$n.iter, ncol = dim(mu_tc1_obs_g1_lnorm)[2])
TC2_obs_g0_rep_lnorm <- matrix(NA, nrow = jagsModel_lnorm$n.iter, ncol = dim(mu_tc2_obs_g0_lnorm)[2])
TC2_obs_g1_rep_lnorm <- matrix(NA, nrow = jagsModel_lnorm$n.iter, ncol = dim(mu_tc2_obs_g1_lnorm)[2])
set.seed(8361299)
for(i in 1:jagsModel_lnorm$n.iter){
  TC1_obs_g0_rep_lnorm[i,] <- rlnorm(dim(mu_tc1_obs_g0_lnorm)[2], mu_tc1_obs_g0_lnorm[i,], sd_tc1_obs_g0_lnorm[i])
  TC1_obs_g1_rep_lnorm[i,] <- rlnorm(dim(mu_tc1_obs_g1_lnorm)[2], mu_tc1_obs_g1_lnorm[i,], sd_tc1_obs_g1_lnorm[i])
  TC2_obs_g0_rep_lnorm[i,] <- rlnorm(dim(mu_tc2_obs_g0_lnorm)[2], mu_tc2_obs_g0_lnorm[i,], sd_tc2_obs_g0_lnorm[i])
  TC2_obs_g1_rep_lnorm[i,] <- rlnorm(dim(mu_tc2_obs_g1_lnorm)[2], mu_tc2_obs_g1_lnorm[i,], sd_tc2_obs_g1_lnorm[i])
}
mu_tc1_obs_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC1[,is.na(TC_1)==FALSE & group==0]
mu_tc1_obs_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC1[,is.na(TC_1)==FALSE & group==1]
mu_tc2_obs_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC2[,is.na(TC_2)==FALSE & group==0]
mu_tc2_obs_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC2[,is.na(TC_2)==FALSE & group==1]
tau_tc1_obs_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC1[,is.na(TC_1)==FALSE & group==0]
tau_tc1_obs_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC1[,is.na(TC_1)==FALSE & group==1]
tau_tc2_obs_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC2[,is.na(TC_2)==FALSE & group==0]
tau_tc2_obs_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC2[,is.na(TC_2)==FALSE & group==1]
TC1_obs_g0_rep_gamma <- matrix(NA, nrow = jagsModel_gamma$n.iter, ncol = dim(mu_tc1_obs_g0_gamma)[2])
TC1_obs_g1_rep_gamma <- matrix(NA, nrow = jagsModel_gamma$n.iter, ncol = dim(mu_tc1_obs_g1_gamma)[2])
TC2_obs_g0_rep_gamma <- matrix(NA, nrow = jagsModel_gamma$n.iter, ncol = dim(mu_tc2_obs_g0_gamma)[2])
TC2_obs_g1_rep_gamma <- matrix(NA, nrow = jagsModel_gamma$n.iter, ncol = dim(mu_tc2_obs_g1_gamma)[2])
set.seed(8361299)
for(i in 1:jagsModel_gamma$n.iter){
  TC1_obs_g0_rep_gamma[i,] <- rgamma(dim(mu_tc1_obs_g0_gamma)[2], shape=mu_tc1_obs_g0_gamma[i,]*tau_tc1_obs_g0_gamma[i,], rate=tau_tc1_obs_g0_gamma[i,])
  TC1_obs_g1_rep_gamma[i,] <- rgamma(dim(mu_tc1_obs_g1_gamma)[2], shape=mu_tc1_obs_g1_gamma[i,]*tau_tc1_obs_g1_gamma[i,], rate=tau_tc1_obs_g1_gamma[i,])
  TC2_obs_g0_rep_gamma[i,] <- rgamma(dim(mu_tc2_obs_g0_gamma)[2], shape=mu_tc2_obs_g0_gamma[i,]*tau_tc2_obs_g0_gamma[i,], rate=tau_tc2_obs_g0_gamma[i,])
  TC2_obs_g1_rep_gamma[i,] <- rgamma(dim(mu_tc2_obs_g1_gamma)[2], shape=mu_tc2_obs_g1_gamma[i,]*tau_tc2_obs_g1_gamma[i,], rate=tau_tc2_obs_g1_gamma[i,])
}

#draw plots
color_scheme_set("red")
ppc_dens_overlay(TC1_g0_obs,TC1_obs_g0_rep_norm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC1_g0_obs,TC1_obs_g0_rep_norm[1:25,])
ppc_hist(TC1_g0_obs,TC1_obs_g0_rep_norm[1:15,])
ppc_dens_overlay(TC2_g0_obs,TC2_obs_g0_rep_norm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC2_g0_obs,TC2_obs_g0_rep_norm[1:25,])
ppc_hist(TC2_g0_obs,TC2_obs_g0_rep_norm[1:15,])
ppc_dens_overlay(TC1_g0_obs,TC1_obs_g0_rep_lnorm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC1_g0_obs,TC1_obs_g0_rep_lnorm[1:25,])
ppc_hist(TC1_g0_obs,TC1_obs_g0_rep_lnorm[1:15,])
ppc_dens_overlay(TC2_g0_obs,TC2_obs_g0_rep_lnorm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC2_g0_obs,TC2_obs_g0_rep_lnorm[1:25,])
ppc_hist(TC2_g0_obs,TC2_obs_g0_rep_lnorm[1:15,])
ppc_dens_overlay(TC1_g0_obs,TC1_obs_g0_rep_gamma[1:25,], bw=0.25)
ppc_ecdf_overlay(TC1_g0_obs,TC1_obs_g0_rep_gamma[1:25,])
ppc_hist(TC1_g0_obs,TC1_obs_g0_rep_gamma[1:15,])
ppc_dens_overlay(TC2_g0_obs,TC2_obs_g0_rep_gamma[1:25,], bw=0.25)
ppc_ecdf_overlay(TC2_g0_obs,TC2_obs_g0_rep_gamma[1:25,])
ppc_hist(TC2_g0_obs,TC2_obs_g0_rep_gamma[1:15,])

color_scheme_set("blue")
ppc_dens_overlay(TC1_g1_obs,TC1_obs_g1_rep_norm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC1_g1_obs,TC1_obs_g1_rep_norm[1:25,])
ppc_hist(TC1_g1_obs,TC1_obs_g1_rep_norm[1:15,])
ppc_dens_overlay(TC2_g1_obs,TC2_obs_g1_rep_norm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC2_g1_obs,TC2_obs_g1_rep_norm[1:25,])
ppc_hist(TC2_g1_obs,TC2_obs_g1_rep_norm[1:15,])
ppc_dens_overlay(TC1_g1_obs,TC1_obs_g1_rep_lnorm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC1_g1_obs,TC1_obs_g1_rep_lnorm[1:25,])
ppc_hist(TC1_g1_obs,TC1_obs_g1_rep_lnorm[1:15,])
ppc_dens_overlay(TC2_g1_obs,TC2_obs_g1_rep_lnorm[1:25,], bw=0.25)
ppc_ecdf_overlay(TC2_g1_obs,TC2_obs_g1_rep_lnorm[1:25,])
ppc_hist(TC2_g1_obs,TC2_obs_g1_rep_lnorm[1:15,])
ppc_dens_overlay(TC1_g1_obs,TC1_obs_g1_rep_gamma[1:25,], bw=0.25)
ppc_ecdf_overlay(TC1_g1_obs,TC1_obs_g1_rep_gamma[1:25,])
ppc_hist(TC1_g1_obs,TC1_obs_g1_rep_gamma[1:15,])
ppc_dens_overlay(TC2_g1_obs,TC2_obs_g1_rep_gamma[1:25,], bw=0.25)
ppc_ecdf_overlay(TC2_g1_obs,TC2_obs_g1_rep_gamma[1:25,])
ppc_hist(TC2_g1_obs,TC2_obs_g1_rep_gamma[1:15,])

#generate posterior means by MC 
set.seed(8361299)
L <- 10000
#approximate marginal mean outcome per group from each model
mu_tc1_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC1[,group==0]
mu_tc1_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC1[,group==1]
mu_tc2_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC2[,group==0]
mu_tc2_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$mu_TC2[,group==1]
sd_tc1_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC1[,1]
sd_tc1_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC1[,2]
sd_tc2_g0_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC2[,1]
sd_tc2_g1_norm <- jagsModel_norm$BUGSoutput$sims.list$sd_TC2[,2]
mmu_tc1_g0_norm <- mmu_tc1_g1_norm <- c()
mmu_tc2_g0_norm <- mmu_tc2_g1_norm <- c()
for(i in 1:jagsModel_norm$n.iter){
    mmu_tc1_g0_norm[i] <- mean(rnorm(L, mu_tc1_g0_norm[i,], sd_tc1_g0_norm[i]))
    mmu_tc1_g1_norm[i] <- mean(rnorm(L, mu_tc1_g1_norm[i,], sd_tc1_g1_norm[i]))
    mmu_tc2_g0_norm[i] <- mean(rnorm(L, mu_tc2_g0_norm[i,], sd_tc2_g0_norm[i]))
    mmu_tc2_g1_norm[i] <- mean(rnorm(L, mu_tc2_g1_norm[i,], sd_tc2_g1_norm[i]))
}
set.seed(8361299)
mu_tc1_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC1[,group==0]
mu_tc1_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC1[,group==1]
mu_tc2_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC2[,group==0]
mu_tc2_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$mu_TC2[,group==1]
sd_tc1_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC1[,1]
sd_tc1_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC1[,2]
sd_tc2_g0_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC2[,1]
sd_tc2_g1_lnorm <- jagsModel_lnorm$BUGSoutput$sims.list$sd_TC2[,2]
mmu_tc1_g0_lnorm <- mmu_tc1_g1_lnorm <- c()
mmu_tc2_g0_lnorm <- mmu_tc2_g1_lnorm <- c()
for(i in 1:jagsModel_lnorm$n.iter){
    mmu_tc1_g0_lnorm[i] <- mean(rlnorm(L, mu_tc1_g0_lnorm[i,], sd_tc1_g0_lnorm[i]))
    mmu_tc1_g1_lnorm[i] <- mean(rlnorm(L, mu_tc1_g1_lnorm[i,], sd_tc1_g1_lnorm[i]))
    mmu_tc2_g0_lnorm[i] <- mean(rlnorm(L, mu_tc2_g0_lnorm[i,], sd_tc2_g0_lnorm[i]))
    mmu_tc2_g1_lnorm[i] <- mean(rlnorm(L, mu_tc2_g1_lnorm[i,], sd_tc2_g1_lnorm[i]))
}
set.seed(8361299)
mu_tc1_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC1[,group==0]
mu_tc1_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC1[,group==1]
mu_tc2_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC2[,group==0]
mu_tc2_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$mu_TC2[,group==1]
tau_tc1_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC1[,group==0]
tau_tc1_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC1[,group==1]
tau_tc2_g0_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC2[,group==0]
tau_tc2_g1_gamma <- jagsModel_gamma$BUGSoutput$sims.list$tau_TC2[,group==1]
mmu_tc1_g0_gamma <- mmu_tc1_g1_gamma <- c()
mmu_tc2_g0_gamma <- mmu_tc2_g1_gamma <- c()
for(i in 1:jagsModel_gamma$n.iter){
  mmu_tc1_g0_gamma[i] <- mean(rgamma(L, shape=mu_tc1_g0_gamma[i,]*tau_tc1_g0_gamma[i,], rate=tau_tc1_g0_gamma[i,]))
  mmu_tc1_g1_gamma[i] <- mean(rgamma(L, shape=mu_tc1_g1_gamma[i,]*tau_tc1_g1_gamma[i,], rate=tau_tc1_g1_gamma[i,]))
  mmu_tc2_g0_gamma[i] <- mean(rgamma(L, shape=mu_tc2_g0_gamma[i,]*tau_tc2_g0_gamma[i,], rate=tau_tc2_g0_gamma[i,]))
  mmu_tc2_g1_gamma[i] <- mean(rgamma(L, shape=mu_tc2_g1_gamma[i,]*tau_tc2_g1_gamma[i,], rate=tau_tc2_g1_gamma[i,]))
}
...
L <- 10000
#approximate marginal mean outcome per group from each model
mu_eq5d0_g0_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_EQ5D0[,group==0]
mu_eq5d0_g1_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_EQ5D0[,group==1]
mu_eq5d1_g0_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_EQ5D1[,group==0]
mu_eq5d1_g1_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_EQ5D1[,group==1]
mu_eq5d2_g0_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_EQ5D2[,group==0]
mu_eq5d2_g1_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_EQ5D2[,group==1]
sd_eq5d0_g0_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$sd_EQ5D0
sd_eq5d0_g1_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$sd_EQ5D0
sd_eq5d1_g0_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$sd_EQ5D1
sd_eq5d1_g1_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$sd_EQ5D1
sd_eq5d2_g0_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$sd_EQ5D2
sd_eq5d2_g1_norm <- jagsModel_gamma_final$BUGSoutput$sims.list$sd_EQ5D2
...
set.seed(8361299)
mu_tc1_g0_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_TC1[,group==0]
mu_tc1_g1_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_TC1[,group==1]
mu_tc2_g0_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_TC2[,group==0]
mu_tc2_g1_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$mu_TC2[,group==1]
tau_tc1_g0_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$tau_TC1[,group==0]
tau_tc1_g1_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$tau_TC1[,group==1]
tau_tc2_g0_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$tau_TC2[,group==0]
tau_tc2_g1_gamma <- jagsModel_gamma_final$BUGSoutput$sims.list$tau_TC2[,group==1]
mmu_tc1_g0_gamma <- mmu_tc1_g1_gamma <- c()
mmu_tc2_g0_gamma <- mmu_tc2_g1_gamma <- c()
for(i in 1:jagsModel_gamma_final$n.iter){
  mmu_tc1_g0_gamma[i] <- mean(rgamma(L, shape=mu_tc1_g0_gamma[i,]*tau_tc1_g0_gamma[i,], rate=tau_tc1_g0_gamma[i,]))
...
#compare mean results for total aggregated mean costs (obtain results on original scale by *1000)
mmu_tc_g0_norm <- mmu_tc1_g0_norm + mmu_tc2_g0_norm
mmu_tc_g1_norm <- mmu_tc1_g1_norm + mmu_tc2_g1_norm
mmu_tc_g0_lnorm <- mmu_tc1_g0_lnorm + mmu_tc2_g0_lnorm
mmu_tc_g1_lnorm <- mmu_tc1_g1_lnorm + mmu_tc2_g1_lnorm
mmu_tc_g0_gamma <- mmu_tc1_g0_gamma + mmu_tc2_g0_gamma
mmu_tc_g1_gamma <- mmu_tc1_g1_gamma + mmu_tc2_g1_gamma
...
res_s2_method1_norm <- list("jagsModel_norm"=jagsModel_norm,
                       ...
                       "TC1_g0_obs"=TC1_g0_obs, "TC1_g1_obs"=TC1_g1_obs, "TC2_g0_obs"=TC2_g0_obs,"TC2_g1_obs"=TC2_g1_obs,
                       "TC1_obs_g0_rep_norm"=TC1_obs_g0_rep_norm, "TC1_obs_g1_rep_norm"=TC1_obs_g1_rep_norm,
                       "TC2_obs_g0_rep_norm"=TC2_obs_g0_rep_norm, "TC2_obs_g1_rep_norm"=TC2_obs_g1_rep_norm,
                       "mmu_tc1_g0_norm"=mmu_tc1_g0_norm,"mmu_tc1_g1_norm"=mmu_tc1_g1_norm,
                       "mmu_tc2_g0_norm"=mmu_tc2_g0_norm, "mmu_tc2_g1_norm"=mmu_tc2_g1_norm,
                       "mmu_tc_g0_norm"=mmu_tc_g0_norm,"mmu_tc_g1_norm"=mmu_tc_g1_norm,
                       "delta_tc_norm"=delta_tc_norm)
res_s2_method1_lnorm <- list("jagsModel_lnorm"=jagsModel_lnorm,
                            "ic_waic_lnorm"=ic_waic_lnorm, "ic_looic_lnorm"=ic_waic_lnorm,
                            "TC1_g0_obs"=TC1_g0_obs, "TC1_g1_obs"=TC1_g1_obs, "TC2_g0_obs"=TC2_g0_obs,"TC2_g1_obs"=TC2_g1_obs,
                            "TC1_obs_g0_rep_lnorm"=TC1_obs_g0_rep_lnorm, "TC1_obs_g1_rep_lnorm"=TC1_obs_g1_rep_lnorm,
                            "TC2_obs_g0_rep_lnorm"=TC2_obs_g0_rep_lnorm, "TC2_obs_g1_rep_lnorm"=TC2_obs_g1_rep_lnorm,
                            "mmu_tc1_g0_lnorm"=mmu_tc1_g0_lnorm,"mmu_tc1_g1_lnorm"=mmu_tc1_g1_lnorm,
                            "mmu_tc2_g0_lnorm"=mmu_tc2_g0_lnorm, "mmu_tc2_g1_lnorm"=mmu_tc2_g1_lnorm,
                            "mmu_tc_g0_lnorm"=mmu_tc_g0_lnorm,"mmu_tc_g1_lnorm"=mmu_tc_g1_lnorm,
                            "delta_tc_lnorm"=delta_tc_lnorm)
res_s2_method1_gamma <- list("jagsModel_gamma"=jagsModel_gamma,
                            "ic_waic_gamma"=ic_waic_gamma, "ic_looic_gamma"=ic_waic_gamma,
                            "TC1_g0_obs"=TC1_g0_obs, "TC1_g1_obs"=TC1_g1_obs, "TC2_g0_obs"=TC2_g0_obs,"TC2_g1_obs"=TC2_g1_obs,
                            "TC1_obs_g0_rep_gamma"=TC1_obs_g0_rep_gamma, "TC1_obs_g1_rep_gamma"=TC1_obs_g1_rep_gamma,
                            "TC2_obs_g0_rep_gamma"=TC2_obs_g0_rep_gamma, "TC2_obs_g1_rep_gamma"=TC2_obs_g1_rep_gamma,
                            "mmu_tc1_g0_gamma"=mmu_tc1_g0_gamma,"mmu_tc1_g1_gamma"=mmu_tc1_g1_gamma,
                            "mmu_tc2_g0_gamma"=mmu_tc2_g0_gamma, "mmu_tc2_g1_gamma"=mmu_tc2_g1_gamma,
                            "mmu_tc_g0_gamma"=mmu_tc_g0_gamma,"mmu_tc_g1_gamma"=mmu_tc_g1_gamma,
                            "delta_tc_gamma"=delta_tc_gamma)
res_s2_method1 <- list("res_s2_method1_norm"=res_s2_method1_norm,
                       "res_s2_method1_lnorm"=res_s2_method1_lnorm,
                       "res_s2_method1_gamma"=res_s2_method1_gamma)
#saveRDS(res_s2_method1,"res_s2_method1.rds")

#final joint model
res_s2_method1_gamma_final <- list("jagsModel_gamma_final"=jagsModel_gamma_final,
                                   "mmu_tc_g0_gamma"=mmu_tc_g0_gamma,"mmu_tc_g1_gamma"=mmu_tc_g1_gamma,
                                   ...
                                   "mmu_tc1_g0_gamma"=mmu_tc1_g0_gamma,"mmu_tc2_g0_gamma"=mmu_tc2_g0_gamma,
                                   "mmu_tc1_g1_gamma"=mmu_tc1_g1_gamma,"mmu_tc2_g1_gamma"=mmu_tc2_g1_gamma,
...
#saveRDS(res_s2_method1_gamma_final,"res_s2_method1_gamma_final.rds")


################################################ version 2 - impute NAs in HRU as 0 (partially-observed) 

N <- length(PBS_dataset_quest_cc_hru_AD$pid)
TC_1 <- PBS_dataset_quest_cc_hru_AD$AG_C1_imp/1000
TC_2 <- PBS_dataset_quest_cc_hru_AD$AG_C2_imp/1000
group <- PBS_dataset_quest_cc_hru_AD$group
...
inits1 <- list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = 1)
inits2 <- list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = 2)
params<-c("alpha0","alpha1","alpha2","sd_TC1","sd_TC2","mu_TC1","mu_TC2","tau_TC1","tau_TC2","log_lik_TC1","log_lik_TC2")
...
#store results
res_s2_method2_norm <- list("jagsModel_norm"=jagsModel_norm,
                            "ic_waic_norm"=ic_waic_norm, "ic_looic_norm"=ic_waic_norm,
                            "TC1_g0_obs"=TC1_g0_obs, "TC1_g1_obs"=TC1_g1_obs, "TC2_g0_obs"=TC2_g0_obs,"TC2_g1_obs"=TC2_g1_obs,
                            "TC1_obs_g0_rep_norm"=TC1_obs_g0_rep_norm, "TC1_obs_g1_rep_norm"=TC1_obs_g1_rep_norm,
                            "TC2_obs_g0_rep_norm"=TC2_obs_g0_rep_norm, "TC2_obs_g1_rep_norm"=TC2_obs_g1_rep_norm,
                            ...
                            "mmu_tc_g0_norm"=mmu_tc_g0_norm,"mmu_tc_g1_norm"=mmu_tc_g1_norm,
                            "delta_tc_norm"=delta_tc_norm)
res_s2_method2_lnorm <- list("jagsModel_lnorm"=jagsModel_lnorm,
                             "ic_waic_lnorm"=ic_waic_lnorm, "ic_looic_lnorm"=ic_waic_lnorm,
                             "TC1_g0_obs"=TC1_g0_obs, "TC1_g1_obs"=TC1_g1_obs, "TC2_g0_obs"=TC2_g0_obs,"TC2_g1_obs"=TC2_g1_obs,
                             "TC1_obs_g0_rep_lnorm"=TC1_obs_g0_rep_lnorm, "TC1_obs_g1_rep_lnorm"=TC1_obs_g1_rep_lnorm,
                             "TC2_obs_g0_rep_lnorm"=TC2_obs_g0_rep_lnorm, "TC2_obs_g1_rep_lnorm"=TC2_obs_g1_rep_lnorm,
                             ...
                             "mmu_tc_g0_lnorm"=mmu_tc_g0_lnorm,"mmu_tc_g1_lnorm"=mmu_tc_g1_lnorm,
                             "delta_tc_lnorm"=delta_tc_lnorm)
res_s2_method2_gamma <- list("jagsModel_gamma"=jagsModel_gamma,
                             "ic_waic_gamma"=ic_waic_gamma, "ic_looic_gamma"=ic_waic_gamma,
                             "TC1_g0_obs"=TC1_g0_obs, "TC1_g1_obs"=TC1_g1_obs, "TC2_g0_obs"=TC2_g0_obs,"TC2_g1_obs"=TC2_g1_obs,
                             "TC1_obs_g0_rep_gamma"=TC1_obs_g0_rep_gamma, "TC1_obs_g1_rep_gamma"=TC1_obs_g1_rep_gamma,
                             "TC2_obs_g0_rep_gamma"=TC2_obs_g0_rep_gamma, "TC2_obs_g1_rep_gamma"=TC2_obs_g1_rep_gamma,
...
res_s2_method2 <- list("res_s2_method2_norm"=res_s2_method2_norm,
                       "res_s2_method2_lnorm"=res_s2_method2_lnorm,
                       "res_s2_method2_gamma"=res_s2_method2_gamma)
#saveRDS(res_s2_method2,"res_s2_method2.rds")

#final joint model
res_s2_method2_gamma_final <- list("jagsModel_gamma_final"=jagsModel_gamma_final,
                                   "mmu_tc_g0_gamma"=mmu_tc_g0_gamma,"mmu_tc_g1_gamma"=mmu_tc_g1_gamma,
                                   "delta_tc_gamma"=delta_tc_gamma, "mmu_qaly_g0_norm"=mmu_qaly_g0_norm,
                                   "mmu_qaly_g1_norm"=mmu_qaly_g1_norm, "delta_qaly_norm"=delta_qaly_norm,
                                   "mmu_tc1_g0_gamma"=mmu_tc1_g0_gamma,"mmu_tc2_g0_gamma"=mmu_tc2_g0_gamma,
                                   ...
                                   "mmu_eq5d1_g0_norm"=mmu_eq5d1_g0_norm,"mmu_eq5d1_g1_norm"=mmu_eq5d1_g1_norm,
                                   "mmu_eq5d2_g0_norm"=mmu_eq5d2_g0_norm,"mmu_eq5d2_g1_norm"=mmu_eq5d2_g1_norm)
#saveRDS(res_s2_method2_gamma_final,"res_s2_method2_gamma_final.rds")



#RESULTS - MODEL LEVEL OF U/C
#load results and extract estimates
library(HDInterval)
res_s2_method1 <- readRDS("res_s2_method1.rds")
res_s2_method2 <- readRDS("res_s2_method2.rds")

#m1 = remove all partially-observed HRU data to construct time point cost data
#amount missing resulting in c
summary(PBS_dataset_quest_cc_hru_AD$AG_C0)
summary(PBS_dataset_quest_cc_hru_AD$AG_C1)
summary(PBS_dataset_quest_cc_hru_AD$AG_C2)

#check empirical distributions of time point cost data by arm
hist(PBS_dataset_quest_cc_hru_AD$AG_C0[PBS_dataset_quest_cc_hru_AD$group==0])
hist(PBS_dataset_quest_cc_hru_AD$AG_C0[PBS_dataset_quest_cc_hru_AD$group==1])
summary(PBS_dataset_quest_cc_hru_AD$AG_C0[PBS_dataset_quest_cc_hru_AD$group==0])
summary(PBS_dataset_quest_cc_hru_AD$AG_C0[PBS_dataset_quest_cc_hru_AD$group==1])
boxplot(PBS_dataset_quest_cc_hru_AD$AG_C0~PBS_dataset_quest_cc_hru_AD$group)
hist(PBS_dataset_quest_cc_hru_AD$AG_C1[PBS_dataset_quest_cc_hru_AD$group==0])
hist(PBS_dataset_quest_cc_hru_AD$AG_C1[PBS_dataset_quest_cc_hru_AD$group==1])
summary(PBS_dataset_quest_cc_hru_AD$AG_C1[PBS_dataset_quest_cc_hru_AD$group==0])
summary(PBS_dataset_quest_cc_hru_AD$AG_C1[PBS_dataset_quest_cc_hru_AD$group==1])
boxplot(PBS_dataset_quest_cc_hru_AD$AG_C1~PBS_dataset_quest_cc_hru_AD$group)
hist(PBS_dataset_quest_cc_hru_AD$AG_C2[PBS_dataset_quest_cc_hru_AD$group==0])
hist(PBS_dataset_quest_cc_hru_AD$AG_C2[PBS_dataset_quest_cc_hru_AD$group==1])
summary(PBS_dataset_quest_cc_hru_AD$AG_C2[PBS_dataset_quest_cc_hru_AD$group==0])
summary(PBS_dataset_quest_cc_hru_AD$AG_C2[PBS_dataset_quest_cc_hru_AD$group==1])
boxplot(PBS_dataset_quest_cc_hru_AD$AG_C2~PBS_dataset_quest_cc_hru_AD$group)

s2m1_mu_c0t1_gamma <- res_s2_method1$res_s2_method1_gamma$mmu_tc1_g0_gamma*1000
s2m1_mu_c1t1_gamma <- res_s2_method1$res_s2_method1_gamma$mmu_tc1_g1_gamma*1000
s2m1_mu_c0t2_gamma <- res_s2_method1$res_s2_method1_gamma$mmu_tc2_g0_gamma*1000
s2m1_mu_c1t2_gamma <- res_s2_method1$res_s2_method1_gamma$mmu_tc2_g1_gamma*1000
s2m1_mu_tc0_gamma <- s2m1_mu_c0t1_gamma + s2m1_mu_c0t2_gamma
s2m1_mu_tc1_gamma <- s2m1_mu_c1t1_gamma + s2m1_mu_c1t2_gamma
s2m1_delta_tc_gamma <- s2m1_mu_tc1_gamma - s2m1_mu_tc0_gamma

c(mean(s2m1_mu_c0t1_gamma),hdi(s2m1_mu_c0t1_gamma, credMass = 0.95))
c(mean(s2m1_mu_c0t2_gamma),hdi(s2m1_mu_c0t2_gamma, credMass = 0.95))
c(mean(s2m1_mu_c1t1_gamma),hdi(s2m1_mu_c1t1_gamma, credMass = 0.95))
c(mean(s2m1_mu_c1t2_gamma),hdi(s2m1_mu_c1t2_gamma, credMass = 0.95))
c(mean(s2m1_mu_tc0_gamma),hdi(s2m1_mu_tc0_gamma, credMass = 0.95))
c(mean(s2m1_mu_tc1_gamma),hdi(s2m1_mu_tc1_gamma, credMass = 0.95))
c(mean(s2m1_delta_tc_gamma),hdi(s2m1_delta_tc_gamma, credMass = 0.95))
mean_m1_delta_tc_gamma <- mean(s2m1_delta_tc_gamma)
ci_m1_delta_tc_gamma <- hdi(s2m1_delta_tc_gamma, credMass = 0.95)

#m2 = impute as 0 all partially-observed HRU to construct c
#amount missing resulting in c
summary(PBS_dataset_quest_cc_hru_AD$AG_C0_imp)
summary(PBS_dataset_quest_cc_hru_AD$AG_C1_imp)
summary(PBS_dataset_quest_cc_hru_AD$AG_C2_imp)

#check empirical distributions of time point cost data by arm
hist(PBS_dataset_quest_cc_hru_AD$AG_C0_imp[PBS_dataset_quest_cc_hru_AD$group==0])
hist(PBS_dataset_quest_cc_hru_AD$AG_C0_imp[PBS_dataset_quest_cc_hru_AD$group==1])
summary(PBS_dataset_quest_cc_hru_AD$AG_C0_imp[PBS_dataset_quest_cc_hru_AD$group==0])
summary(PBS_dataset_quest_cc_hru_AD$AG_C0_imp[PBS_dataset_quest_cc_hru_AD$group==1])
boxplot(PBS_dataset_quest_cc_hru_AD$AG_C0_imp~PBS_dataset_quest_cc_hru_AD$group)
hist(PBS_dataset_quest_cc_hru_AD$AG_C1_imp[PBS_dataset_quest_cc_hru_AD$group==0])
hist(PBS_dataset_quest_cc_hru_AD$AG_C1_imp[PBS_dataset_quest_cc_hru_AD$group==1])
summary(PBS_dataset_quest_cc_hru_AD$AG_C1_imp[PBS_dataset_quest_cc_hru_AD$group==0])
summary(PBS_dataset_quest_cc_hru_AD$AG_C1_imp[PBS_dataset_quest_cc_hru_AD$group==1])
boxplot(PBS_dataset_quest_cc_hru_AD$AG_C1_imp~PBS_dataset_quest_cc_hru_AD$group)
hist(PBS_dataset_quest_cc_hru_AD$AG_C2_imp[PBS_dataset_quest_cc_hru_AD$group==0])
hist(PBS_dataset_quest_cc_hru_AD$AG_C2_imp[PBS_dataset_quest_cc_hru_AD$group==1])
summary(PBS_dataset_quest_cc_hru_AD$AG_C2_imp[PBS_dataset_quest_cc_hru_AD$group==0])
summary(PBS_dataset_quest_cc_hru_AD$AG_C2_imp[PBS_dataset_quest_cc_hru_AD$group==1])
boxplot(PBS_dataset_quest_cc_hru_AD$AG_C2_imp~PBS_dataset_quest_cc_hru_AD$group)

s2m2_mu_c0t1_gamma <- res_s2_method2$res_s2_method2_gamma$mmu_tc1_g0_gamma*1000
s2m2_mu_c1t1_gamma <- res_s2_method2$res_s2_method2_gamma$mmu_tc1_g1_gamma*1000
s2m2_mu_c0t2_gamma <- res_s2_method2$res_s2_method2_gamma$mmu_tc2_g0_gamma*1000
s2m2_mu_c1t2_gamma <- res_s2_method2$res_s2_method2_gamma$mmu_tc2_g1_gamma*1000
s2m2_mu_tc0_gamma <- s2m2_mu_c0t1_gamma + s2m2_mu_c0t2_gamma
s2m2_mu_tc1_gamma <- s2m2_mu_c1t1_gamma + s2m2_mu_c1t2_gamma
s2m2_delta_tc_gamma <- s2m2_mu_tc1_gamma - s2m2_mu_tc0_gamma

c(mean(s2m2_mu_c0t1_gamma),hdi(s2m2_mu_c0t1_gamma, credMass = 0.95))
c(mean(s2m2_mu_c0t2_gamma),hdi(s2m2_mu_c0t2_gamma, credMass = 0.95))
c(mean(s2m2_mu_c1t1_gamma),hdi(s2m2_mu_c1t1_gamma, credMass = 0.95))
c(mean(s2m2_mu_c1t2_gamma),hdi(s2m2_mu_c1t2_gamma, credMass = 0.95))
c(mean(s2m2_mu_tc0_gamma),hdi(s2m2_mu_tc0_gamma, credMass = 0.95))
c(mean(s2m2_mu_tc1_gamma),hdi(s2m2_mu_tc1_gamma, credMass = 0.95))
c(mean(s2m2_delta_tc_gamma),hdi(s2m2_delta_tc_gamma, credMass = 0.95))
mean_m2_delta_tc_gamma <- mean(s2m2_delta_tc_gamma)
ci_m2_delta_tc_gamma <- hdi(s2m2_delta_tc_gamma, credMass = 0.95)

#construct ggdataframe for plotting comparison
df_s2 <- data.frame(
  "method" <- c("m1","m2"),
  "estimate" <- c(mean_m1_delta_tc_gamma,mean_m2_delta_tc_gamma),
  "lb" <- c(ci_m1_delta_tc_gamma[1],ci_m2_delta_tc_gamma[1]),
  "ub" <- c(ci_m1_delta_tc_gamma[2],ci_m2_delta_tc_gamma[2])
)
names(df_s2) <- c("method","estimate","lb","ub")
df_s2$methodf <- as.factor(df_s2$method)

ggplot(df_s2, aes(x=methodf, y=estimate)) + 
